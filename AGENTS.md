...
> **Purpose**: This is a personal-use tool (not a commercial product) being developed to help **Gaya Liz** study. It started from the **Shadcn Dashboard Free** template and is being repurposed as a study companion. Keep that context in mind when making product decisions — prioritize practical study features and simplicity over template/boilerplate parity.
...
- Composes `@eslint/js` recommended, `typescript-eslint` recommended and `eslint-plugin-react-hooks` recommended
...
1. **Personal study tool, not a product** — this repo exists to help Gaya Liz study (e.g. tracking the TCDF edital in `data/edital.json`). Favor features that serve that goal over generic template parity.
2. **Verify feature claims against code, not the dependency list.** Grep `src/` before telling the user a feature exists.
3. **This is a client-only SPA** — there are no server actions, API routes, or server components. All "backend" behavior is MSW-mocked.
...
6. **Mock data first** — new features should follow the mock-data + MSW-handler + context pattern in `src/api/`, not fetch a real endpoint, unless one already exists.
